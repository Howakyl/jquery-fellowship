...
const makeMiddleEarth = () => {
  ...
  for (let i =0; i < lands.length; i++) {
    let $newLandArticle = $('<article />');
    let $newLandh1 = $(`<h1>${lands[i]}</h1>`);
    $newLandArticle.attr('id',`${lands[i]}`);
    $newLandArticle.append($newLandh1);
    $newSection.append($newLandArticle);
  }
};
...
const makeBaddies = () => {
  ...
  let $baddyUl = $('<ul />');
  let $mordorArticle = $('#Mordor');
  $mordorArticle.append($baddyUl);

  for (let i = 0; i < baddies.length; i++) {
    let $baddyListItem = $(`<li>${baddies[i]}</li>`);
    $baddyListItem.attr('class','baddy');
    $baddyUl.append($baddyListItem);
  }
...
const forgeTheFellowShip = () => {
  ...
  const $fellowshipH1 = $('<h1>The Fellowship</h1>');
  $fellowshipH1.appendTo($theFellowship);
  // 3. append the fellowship to middle-earth
  const $middleEarth = $('#middle-earth');
  $middleEarth.append($theFellowship);
  // 4. add the unordered lists of hobbits and buddies to 'the-fellowship'
  const $hobbitUl = $('#hobbit-ul');
  const $buddyUl = $('.buddy');
  $buddyUl.appendTo($theFellowship);
  $hobbitUl.appendTo($theFellowship);
};
